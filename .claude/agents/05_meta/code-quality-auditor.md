...
| **ii** | Hooks present but `hooks.json` malformed | When the plugin contains a `hooks/` directory, validate `hooks.json` against the canonical Anthropic hook schema (`type: "command"` + non-empty `command` field per [`docs/HOOK_SCHEMA.md`](../../../docs/HOOK_SCHEMA.md)). Catches missing fields, parse errors, empty `hooks` blocks. Reuses the validation logic from `cruft-check.sh` heuristic viii. Confidence: `high`. |
