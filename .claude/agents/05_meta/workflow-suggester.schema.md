...
| `suggested_artifact_type` | string enum (extensible) | What kind of capture would address this pattern. v1.1.0 values: `script`, `skill`, `agent`, `command`, `manual_action`, `unclear`. v1.1.x adds `doc-fix` (added by `cruft-checker` for skeleton-doc cruft observations; resolved manually by the user — no `doc-fix-builder` X-builder ships in v1.1.x). Future X-builders may register new values; consumers ignore unknown values. |
