...
| `suggested_artifact_type` | string enum (extensible) | What kind of capture would address this pattern. v1.1.0 values: `script`, `skill`, `agent`, `command`, `manual_action`, `unclear`. v1.1.x adds `doc-fix` (added by `cruft-checker` heuristics i, ii, iv, v, vii, x for skeleton-doc cruft observations; resolved manually) and `infrastructure-fix` (added by `cruft-checker` heuristic viii for hook-entry config-schema violations; resolved manually — no `infrastructure-fix-builder` X-builder ships in v1.1.x). Future X-builders may register new values; consumers ignore unknown values. |
...
v1.1.0 ships six values. v1.1.x adds two more (`doc-fix` from Phase 7, `infrastructure-fix` from Phase 16) — both produced by `cruft-checker` heuristics, both manually resolved (no X-builder). Future X-builders may add new ones — e.g. `hook` (v1.2+?), `template` (some future), `documentation` (some future). The rules for adding:
