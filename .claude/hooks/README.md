...
- **SessionStart** → three registrations:
  - `sessionstart-rules.sh` — seven jobs in one hook: (1) re-injects durable rules from `compactPrompt`, (2) invokes `.claude/scripts/drift-check.sh` and folds any drift notice into the same `additionalContext` block, (3) invokes `.claude/scripts/task-watchdog.sh` to emit observations from the prior session's transcript, (4) invokes `.claude/scripts/goals-surface.sh --hook` (Phase 68) for at most one due-scheduled-goals line, 24h cooldown, (5) the first-run welcome (Phase 73) — consumes the gitignored `.claude/.first-run` flag install.sh drops and prints a one-time orientation block; fires exactly once per install lifetime, (6) the dispatch-class audit cadence line (Phase 74) — gate-config `audits` registry vs the session counters in `telemetry/audit-state.json`; at most one `[infrastructure-audit]` due line, 24h anti-repeat marker, (7) the live status-strip refresh (Phase 97) — rewrites `receipts/live.html` from on-disk state; file write only, zero output. Failures of the sub-steps are swallowed — the hook always emits cleanly and never blocks session start.
  ...
  - `sessionstart-cost-summary.sh` — one ambient token-spend line from Phase 46 telemetry against `gate-config.json` thresholds, plus the cooldown-gated Phase 53 optimizer reminder line.
