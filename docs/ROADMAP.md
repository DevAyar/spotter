...
- **Observation layer is working.** Independent signal sources feed a shared schema in `.claude/observations/` (the producer roster is enumerated in `session-observer.schema.md`, not here — lists drift, Phase 114/127); the original session-observer retired in Phase 58 (unpopulated surface). New producers can be added without re-architecting consumers.
...
- **Project-level audit triad is working.** `cruft-checker` (doc rot inside the project; 24h cooldown), `drift-checker` (skeleton-version drift against the upstream release; every session), `code-quality-auditor` (installed-plugin sanity; 24h cooldown) all fire at session start.
...
- **Per-project manager-optimizer (centerpiece — SHIPPED v1, Phase 53)** — one instance per installed project; watches two layers: the project's pattern of *AI decisions* and the *recorded decisions at the approval gates*. Routing splits by layer. AI-decision findings draft refinements to that project's `CLAUDE_MANAGER.md` (the AI-facing directive surface). Gate-decision findings draft adjustments to that project's **gate/friction config** (the operation-tier surface — § Appendix; Phase 48 implements), surfaced for the user's approval on the human-facing side (your-view, forthcoming) — never written into `CLAUDE_MANAGER.md`. Both layers draft only; approval-gated autonomy holds across both. The principle behind it is attention economics, stated as engineering: adoption friction is minimized ruthlessly (install, defaults, time-to-first-value); protection friction is priced by risk and reversibility, spends attention only at genuine decision points, and decreases as recorded patterns earn trust — the gate getting smarter, not more annoying.
...
- **`plugin-discovery-agent` (SHIPPED, Phase 76)** — surfaces ecosystem candidates relevant to the project's current state. Ships with `.claude/scripts/plugin-discovery.sh` (the script is the contract): entries enter as candidate/installed only, draft-only, no network, no verdicts.
...
Tiers 1-3 are soft gates; Tier 4 is mixed (operation class configurable, destructive patterns hard-blocked); Tier 5 is a hard safeguard. **How it applies:** new mechanisms classify their operations into one of the five tiers; tier assignment drives gate selection. Phase 48 shipped the cost block + `token-cost-monitor` with the operation-tier / friction slots documented-empty; **Phase 85 shipped the slots' implementation** — the five-class `operation_tiers` block, the three-lane `friction` block (tier_5 structurally absent from the configurable space; the PreToolUse hooks stated as the floor no lane may bypass), template defaults encoding then-current behavior exactly, and the dogfood install as the first tuned copy (docs-only + mechanical-fix classes at `flow_with_receipt`, derivation from the recorded approval pattern in the tuning commit).
