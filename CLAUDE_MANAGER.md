...
**Interaction with plan-mode and `/goals`.** Plan-mode is the natural seam: surface uncertainties in the plan (one line each, named) so the approval gate disposes of them before execution rather than mid-edit. For request-level under-specification (goals, scope), the `/goals` clarifying seam (v1.1+ future hook) is the upstream catch — its absence does not waive this directive; until `/goals` ships, ask inline in plan mode. Counter-pattern to avoid: emitting confident text that contains a hedge ("I believe …", "should be …") about a load-bearing claim — convert the hedge into a verification step or a question.
