...
**The rendered card (Phase 93, appearance Phase 94):** at ship time, after writing the receipt, pipe it through `receipt-render.sh` (stdin or file arg) and name the output path in the report's closing line — "card: .claude/receipts/latest.html"; a dated copy accrues beside it. **Then make the card appear — the three-glass map (Phase 100):** desktop pane = navigate to **`latest-tight.html`**, send ONE scroll-to-top (Ctrl+Home — panes retain scroll position between uses, and this pane's `file:` handling ignores fragment anchors, recon-proven; the tight form still carries `id="top"` for panes that honor them), then the shot. VS Code = open the card or strip in a **Live Preview** tab (one-time extension install; its tab auto-reloads on file change — better than the meta refresh), and the strip pins as an editor tab the same way. Terminal = `receipt-render.sh --show` (auto: VS Code note / TTY ANSI card / silent on CI and pipes) or `--ansi` explicit. Pane capture detail: resize the viewport to the card's size BEFORE shooting (**`resize_window` in this environment; ~760px wide** — or the desktop preset where explicit sizing is ignored), then take ONE screenshot so the card appears inline, edge-to-edge and uncut — the capture form is densified to fit the pane, and a genuinely long receipt overflows honestly below the fold, never truncated (one navigation, one resize, one shot per ship, never ambient — the image entering context costs a small number of tokens; that trade is known and accepted); ELSE run the render with `--open` so the card appears in the user's default browser at the normal centered form (launch suppressed under CI; a local `file://` navigation either way — not a network call). Opt-out: delete this convention line from your manager copy. In-chat, receipts render as **markdown lines with bold field names, not fenced code blocks** — the fields are prose, not code. Hierarchy, stated: **the receipt TEXT in the ship report is the canonical record; the HTML card is a render of it** — regenerable, gitignored, never the source of truth.

**The live strip (Phase 97):** `.claude/receipts/live.html` is a thin pinnable status bar — last ship + sha, the cost line (the cost hook's own output, so the Phase 91 display rules apply verbatim), and due-audit chips (honestly absent when nothing is due). Pin it on Windows with a small browser window + PowerToys Always On Top (Win+Ctrl+T); OBS users can point a browser source at the file. **Event-fresh, plainly:** the strip re-renders when hooks fire — session starts and ship renders — so it is as fresh as the last event, never real-time; the page's 10-second refresh only re-reads the file. Standalone refresh: `receipt-render.sh --live` (with `--open` to launch it); no daemons, no watchers. **The pin recipe (Phase 99):** `receipt-render.sh --live --window` launches the strip chrome-free (browser app mode — no URL bar, no tabs; sized ~900×120 where the browser honors it), then Win+Ctrl+T pins it; the strip stays self-refreshing in app mode — the 10-second refresh is the page's own.
