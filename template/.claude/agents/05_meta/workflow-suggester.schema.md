...
| `suggested_artifact_type` | string enum (extensible) | What kind of capture would address this pattern. v1.1.0 values: `script`, `skill`, `agent`, `command`, `manual_action`, `unclear`. v1.1.x adds `doc-fix` (added by `cruft-checker` heuristics i, ii, iii, iv, v, vii, ix, x for skeleton-doc cruft observations; resolved manually), `infrastructure-fix` (added by `cruft-checker` heuristic viii for hook-entry config-schema violations; resolved manually — no `infrastructure-fix-builder` X-builder ships in v1.1.x), and `lesson` (added v1.1.2; signals a structural insight to codify into the right directive surface like `CLAUDE_MANAGER.md`, `docs/ROADMAP.md`, or the session-handoff doc; resolved manually by the user via small-fix codification commit — no `lesson-builder` X-builder ships). v1.1.4 reuses the existing `manual_action` value as the routing target for `pattern_type: plugin_quality` observations emitted by `code-quality-auditor` (manifest honesty + security hygiene of installed plugins; resolved manually — user reads the capture and remediates by filing a plugin issue, uninstalling, or reconfiguring). Future X-builders may register new values; consumers ignore unknown values. |
...
v1.1.0 ships six values. v1.1.x adds three more (`doc-fix` from Phase 7, `infrastructure-fix` from Phase 16, `lesson` from Phase 18) — all manually resolved (no X-builder ships in v1.1.x). Future X-builders may add new ones — e.g. `hook` (v1.2+?), `template` (some future), `documentation` (some future). The rules for adding:
