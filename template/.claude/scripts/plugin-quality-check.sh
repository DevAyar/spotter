...
# python-first copy. Still exits 0 — this runs from the SessionStart
# chain (--hook) and must never block a session — but says why on stderr
# instead of appearing to have audited (the cruft-check precedent). Exit
# contract unchanged.
# shellcheck source=../lib/detect-python.sh
. "$(cd "$(dirname "$0")/../lib" 2>/dev/null && pwd)/detect-python.sh"
PYBIN="$DETECTED_PYTHON"
if [ -z "$PYBIN" ]; then
  printf 'plugin-quality-check: skipped — %s\n' "$(python_required_message)" >&2
  exit 0
fi
